...
* Summarize **what the data captures** (e.g., agent–environment interactions, simulation outputs, sensor logs).
* Describe its **source** (generated, measured, or derived).
* Clarify **intended use** — e.g., model training, evaluation, visualization.
* Emphasize **semantic meaning**, not technical storage format.

### Example

> The dataset records multi-agent simulation trajectories.
> Each entry corresponds to a single time step, containing the agent’s state vector, chosen action, environment response, and resulting reward.
...
* Describe **hierarchical or relational structure** — one-to-many, many-to-one, etc.
...
> **Relationships:**
>
> * Each `Episode` contains multiple `Steps`, linked via `episode_id`.
> * Each `Step` links to exactly one `Agent` through `agent_id`.
...
| -------------- | ----------- | ---- | ---------------------------------- | ---------------------------- |
| `episode_id`   | int         | —    | Unique identifier for each episode | Unique                       |
| `step_id`      | int         | —    | Index of time step in episode      | Sequential                   |
| `agent_id`     | str         | —    | Agent label                        | Must exist in agent registry |
| `state_vector` | list[float] | —    | Observed environment state         | Length = 12                  |
| `action`       | int         | —    | Chosen discrete action             | [0–9]                        |
| `reward`       | float       | —    | Scalar feedback value              | [−1, 1]                      |
...
> * Each `Episode` has a one-to-many relationship with `Step`, keyed by `episode_id`.
> * Within an episode, `step_id` enforces temporal progression.
> * Each `Step` references its `Agent` via `agent_id`, ensuring ownership.
...
* Outline **processing pipeline** — raw → intermediate → processed.
...
> * **Validation:** Steps with missing rewards are imputed using the most recent valid reward before aggregation.
...
> * Statistical sanity check: mean reward ∈ [−0.2, 0.8].
...
| `normalized_state`  | ( (x - μ) / σ )                | Improves training stability                      |
...
> * No explicit uncertainty fields — planned addition of confidence scores in next version.
